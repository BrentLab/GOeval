...
After a couple minutes, the directory at out_dir should contain a folder of subsets of the original network, a folder that contains the ORA summaries, and a .pdf file with plots of metrics calculated from those summaries. The evaluate function returns the data used to make the plots as a list of DataFrames.

Do not be worried if the graphs output by the above code have weird shapes. The "marbach_extract.tsv" network has very few TFs for the sake of quick computation.

The run time changes with the size of the network and with larger networks may take up to several hours. It is thus recommended to use a computing cluster when working with large networks. It is possible to speed up the process by calling each step manually as shown in the "Advanced usage" section below and running the webgestalt_network function calls in parallel.
\
\
\
Advanced usage:
